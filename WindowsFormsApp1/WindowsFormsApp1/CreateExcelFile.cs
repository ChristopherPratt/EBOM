    ...
    class CreateExcelFile
    {
        ...
        public void saveExcelFile()
        {
             ...
             */
                ...
                for (int row = 1; row < totalRows; row++) // the plus one is because the excel columns and rows start at 1
                    writeData(allCells[row, totalColumns - 1]);
                Console.WriteLine("Finished reading right border");

                for (int column = 0; column < totalColumns - 1; column++) // the plus one is because the excel columns and rows start at 1
                    writeData(allCells[totalRows - 1, column]);
                Console.WriteLine("Finished reading bottom border");

                for (int row = 1; row < totalRows - 1; row++) // the plus one is because the excel columns and rows start at 1
        ...
        public void readExcelFile(int totalRows, int totalColumns)
        {
            ...
            try
            {
                ...
                string excelTemplate = System.AppDomain.CurrentDomain.BaseDirectory + "97101921-02" + ".xlsx";
