...
namespace EBOM_Creation_Tool_v2
{
    class excelFileParser
    {
        ...
        {
            /////////////////////// find and parse tag //////////////////////////
            string tag = parseTag(cell[1,1].Text);
            
            switch (tag)
            {
                case "TextHere":
                    {
                        populateTitleBlockCell(cell[1, 1], ref template3);
                        break;
                    }
                case "HeaderHere":
                    {
                        populateHeaderList(cell[1, 1], ref template3);
                        break;
                    }
                case "BodyHere":
                    {
                        populateBodySectionCell(cell[1, 1], ref template3);
                        break;
                    }
                case "Sort":
                    {
                        parseSort(cell[1, 1], ref sort3);
                        break;
                    }
                case "Footer":
                    {
                        getFooterInfo(cell[1, 1], ref template3.footerColor, ref template3.footerList);
                        break;
                    }
                case "Group":
                    {
                        getQuantityGrouping(column, ref countParts2.groupedColumns);
                        break;
                    }
                case "Quantity":
                    {
                        getQuantityColumn(column, ref countParts2.quantityColumn);
                        break;
                    }
                case "EndRow":
                    {
                        totalRows2 = row; // change loop iterator limit variable for rows if EndRow tag is found
                        break;
                    }
                case "EndColumn": 
                    {
                        totalColumns2 = column; // change loop iterator limit variable for columns if EndColumn tag is found
                        break;
                    }
            }
        }
        ...
        /////////////////////// find and parse tag //////////////////////////
        private string parseTag(string tempText)
        {
            if (tempText.Contains("["))
            {
                string[] tempSplit = tempText.Split('[');
        ...
        public void parseSort (Range cell, ref sort sort4)
        {
            ...
            {                
                ...
                foreach (string tempString in tempDelimiter) sort4.customSort[sort4.customSort.Count - 1].Add(tempString); // should make above cell entry into { {1, *column* , *sortType* P,C,CN,L,R} , {4, *column*, incrementing} }
            }
        }
